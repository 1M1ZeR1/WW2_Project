    ...
    public override void Execute()
    {
        ...
        else
        {
            if (economyScore >= 200 && _spawnedSquads.Count == 0)
            {
                ...
                {
                    ServiceRegistry.WorkWithController<MovementController>().AddMovementForSquad(enemysControllerScript.GetMainBaseCell(), cellNeedToCapture, squad);

                    ServiceRegistry.WorkWithController<MovementController>().AddEvent(squad, SquadHasReached);
                }
            }
        }
    }
...
public class DecisionAction_StreigtheningTerritory:DecisionAction
{
    ...
    private void BuildStreigthening(GameObject cell)
    {
        ...
        if (!cellBuildings.CheckBuildIsBuilt(BuildsEnum.Foxhole))
        {
            foreach(var squad in ServiceRegistry.WorkWithController<GameController>().GetAllEnemysOnCell(cell))
            {
                if (squad is EngineerSquad) { cellBuildings.AddToBuildList_Safely(new FoxholeBuild(), BuildsEnum.Foxhole); TriggerEvent(true,this,cell); Debug.Log("Вырыл окоп"); break; }
            }
        }
        if (!cellBuildings.CheckBuildIsBuilt(BuildsEnum.Fort))
        {
            if(ServiceRegistry.WorkWithController<GameController>().GetAllEnemysOnCell(cell).Count != 0)
            {
                cellBuildings.AddToBuildList_Safely(new FortBuild(), BuildsEnum.Fort);
...
public class DecisionAction_CatchTerritory:DecisionAction,IDecisionAction_WorkWithSquads
{
    ...
    public DecisionAction_CatchTerritory(GameObject mainBase,float maxFinderDistance)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ServiceRegistry.WorkWithController<UnitsSpawner>().SpawnSquadOnCell(SideEnum.Enemys, mainBase);
        }
    ...
    public override void Execute()
    {
        ...
        {
            ServiceRegistry.WorkWithController<MovementController>().AddMovementForSquad(cellsToMove_OneStep.Item1, cellsToMove_OneStep.Item2, squad);

            ServiceRegistry.WorkWithController<MovementController>().AddEvent(squad, (AbstractSquad squad, GameObject cell) =>
            {
