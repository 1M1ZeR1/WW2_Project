...
public class BattleModuleUIScript : MonoBehaviour
{
    ...
    public void StartBattle(GameObject cell)
    {
        ...
        allFunctions.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text = $"Битва за {ServiceRegistry.WorkWithController<CellController>().WorkWithCell<CellParametersHandler>(cell).GetParameter<CellDiscription>().GetCellName()}";
